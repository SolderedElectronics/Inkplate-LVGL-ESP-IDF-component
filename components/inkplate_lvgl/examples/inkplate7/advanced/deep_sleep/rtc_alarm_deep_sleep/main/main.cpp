 ...
 * @brief       RTC alarm deep sleep example for Soldered Inkplate 7 with LVGL.
 *
 * @details     Sets an RTC alarm 60 seconds ahead, displays current time, then
 *              enters deep sleep. The RTC alarm interrupt (GPIO2) wakes the
 *              device and the display refreshes.
 *
 * Requirements:
 * - Board:      Soldered Inkplate 7
 * - Framework:  ESP-IDF v6.x
 * - Hardware:   Inkplate 7, USB cable, CR2032 battery (optional)
 * - Extra:      None
 *
 * Configuration:
 ...
 * - GPIO2 is the RTC INT pin (active-low alarm output) on Inkplate 7SPECTRA.
 *
...
extern "C" void app_main(void) {
    ...
    // GPIO2 = RTC INT pin (active-low) on Inkplate 7SPECTRA
