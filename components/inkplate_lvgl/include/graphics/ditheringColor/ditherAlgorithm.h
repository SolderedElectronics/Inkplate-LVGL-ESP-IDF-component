...
public:
  void begin(uint16_t *palette, uint8_t *paletteIndices, uint8_t paletteSize,
             BoardBase *inkplatePtr);
...
private:
  ...
  RGBTRIPLE map_pixel_classic(int _r, int _g, int _b, uint16_t *palette,
                              uint8_t *palette_indices, uint8_t palette_size);
  RGBTRIPLE map_pixel_fast(int _r, int _g, int _b);
  void RGB_to_HSV(int _r, int _g, int _b, float *h, float *s, float *v);
  float HSV_distance(float h1, float s1, float v1, float h2, float s2,
                     float v2);
  RGBTRIPLE map_pixel_HSV(int _r, int _g, int _b, uint16_t *palette,
                          uint8_t *palette_indices, uint8_t palette_size);
