 ...
 *              colour EPD panels (Inkplate 6COLOR, Inkplate 2, Inkplate
 * 13SPECTRA).
 *
...
/**
 * @brief   Unpack an RGB565 word into R (0–31), G (0–63), B (0–31) channel
 * values.
 */
void DitherAlgorithm::RGB565_to_RGBtriple(uint16_t c, uint8_t *r, uint8_t *g,
                                          uint8_t *b) {
  *r = (c >> 11) & 0x1F;
  *g = (c >> 5) & 0x3F;
  *b = c & 0x1F;
...
/**
 * @brief   Copy the board's colour palette into PSRAM and store the Inkplate
 * pointer. Must be called once from the driver's initDriver() before any
 * dithering.
 */
void DitherAlgorithm::begin(uint16_t *palette, uint8_t *paletteIndices,
                            uint8_t paletteSize, BoardBase *inkplatePtr) {
  palette_size = paletteSize;

  _palette = (uint16_t *)heap_caps_malloc(palette_size * sizeof(uint16_t),
                                          MALLOC_CAP_SPIRAM);
  _paletteIndices = (uint8_t *)heap_caps_malloc(palette_size * sizeof(uint8_t),
                                                MALLOC_CAP_SPIRAM);
...
/**
 * @brief   Find the nearest palette colour using luminance-weighted RGB
 * distance. Weights (R×30, G×59, B×11) approximate the sensitivity of human
 * vision. Sets the global color_index to the matched palette position.
 */
RGBTRIPLE DitherAlgorithm::map_pixel_classic(int _r, int _g, int _b,
                                             uint16_t *palette,
  ...
  for (uint8_t j = 0; j < palette_size; j++) {
    ...
    uint64_t dist = ((uint64_t)(dr * dr) * 30) + ((uint64_t)(dg * dg) * 59) +
                    ((uint64_t)(db * db) * 11);
...
/**
 * @brief   Find the nearest palette colour using unweighted Euclidean RGB
 * distance with a two-level tie-breaking heuristic:
 *            1. Prefer the entry with the closer luma (2R + 5G + B, cheap
 * approximation).
 *            2. At equal luma, prefer the entry whose saturation is closer to
 * the source to avoid washing out saturated colours with a neutral match. Sets
 * color_index to the matched palette position.
 */
  ...
  for (uint8_t j = 0; j < palette_size; j++) {
    ...
    if (dist < min_dist) {
      ...
      int bestY = 2 * ((best_color >> 11) & 0x1F) +
                  5 * ((best_color >> 5) & 0x3F) + (best_color & 0x1F);
      ...
      if (brightnessErr < bestBrightnessErr) {
        ...
        int bestSat =
            abs(bestPr - bestPg) + abs(bestPg - bestPb) + abs(bestPb - bestPr);
 ...
 */
void DitherAlgorithm::RGB_to_HSV(int _r, int _g, int _b, float *h, float *s,
                                 float *v) {
  float r = _r / 31.0f;
  float g = _g / 63.0f;
  float b = _b / 31.0f;
 ...
 */
float DitherAlgorithm::HSV_distance(float h1, float s1, float v1, float h2,
                                    float s2, float v2) {
 ...
 */
RGBTRIPLE DitherAlgorithm::map_pixel_HSV(int _r, int _g, int _b,
                                         uint16_t *palette,
                                         uint8_t *palette_indices,
                                         uint8_t palette_size) {
  float ph, ps, pv;
 ...
 */
uint8_t DitherAlgorithm::clampValue(int32_t _value, int32_t _min,
                                    int32_t _max) {
  if (_value > _max)
    _value = _max;
  if (_value < _min)
 ...
 *
 *          Algorithm overview:
 *            1. Decode the RGB565 framebuffer into a 2-D RGBTRIPLE array in
 * PSRAM.
 ...
 */
void DitherAlgorithm::ditherFramebuffer(uint8_t *frameBuffer, int width,
                                        int height) {

  // Allocate a 2D array for pixels
  RGBTRIPLE **pixels = (RGBTRIPLE **)heap_caps_malloc(
      height * sizeof(RGBTRIPLE *), MALLOC_CAP_SPIRAM);
  if (!pixels) {
    return;
  }

  for (int y = 0; y < height; y++) {
    pixels[y] = (RGBTRIPLE *)heap_caps_malloc(width * sizeof(RGBTRIPLE),
                                              MALLOC_CAP_SPIRAM);
    if (!pixels[y]) {
      for (int i = 0; i < y; i++)
        free(pixels[i]);
  ...
  // Prepare dithering
  int16_t *errCurrR =
      (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
  int16_t *errCurrG =
      (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
  int16_t *errCurrB =
      (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = xStart; x != xEnd; x += direction) {
      ...
      uint16_t colorPicked = _paletteIndices[color_index];
