 ...
 */
void DitherAlgorithm::begin(uint16_t *palette, uint8_t *paletteIndices, uint8_t paletteSize, BoardBase *inkplatePtr)
{
    palette_size = paletteSize;

    _palette = (uint16_t *)heap_caps_malloc(palette_size * sizeof(uint16_t), MALLOC_CAP_SPIRAM);
    _paletteIndices = (uint8_t *)heap_caps_malloc(palette_size * sizeof(uint8_t), MALLOC_CAP_SPIRAM);
...
void DitherAlgorithm::ditherFramebuffer(uint8_t *frameBuffer, int width, int height)
{
    ...
    RGBTRIPLE **pixels = (RGBTRIPLE **)heap_caps_malloc(height * sizeof(RGBTRIPLE *), MALLOC_CAP_SPIRAM);
    if (!pixels)
    {
        return;
    }
    ...
    {
        pixels[y] = (RGBTRIPLE *)heap_caps_malloc(width * sizeof(RGBTRIPLE), MALLOC_CAP_SPIRAM);
        if (!pixels[y])
        {
            for (int i = 0; i < y; i++)
                free(pixels[i]);
    ...
    // Prepare dithering
    int16_t *errCurrR = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    int16_t *errCurrG = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    int16_t *errCurrB = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);

    int16_t *errNextR = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    int16_t *errNextG = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    int16_t *errNextB = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
