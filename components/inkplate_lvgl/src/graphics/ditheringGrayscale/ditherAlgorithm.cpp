...
/**
 * @brief   Store the Inkplate instance pointer for later use by
 * ditherFramebuffer().
 *
 * @param   inkplatePtr
 *          Pointer to the owning Inkplate instance.
 */
void DitherAlgorithm::begin(BoardBase *inkplatePtr) { _inkplate = inkplatePtr; }

/**
 * @brief   Apply Floyd-Steinberg serpentine dithering to the LVGL L8
 * framebuffer and write the result directly into the EPD framebuffer via
 * writePixelInternal().
 *
 ...
 * @param   mode         0 = 1-bit (black and white), 1 = 3-bit (8 grayscale
 * levels).
 */
void DitherAlgorithm::ditherFramebuffer(uint8_t *frameBuffer, int width,
                                        int height, uint8_t mode) {
  // Allocate two row-sized error buffers in PSRAM. errCurr holds the errors
  // being consumed on the current row; errNext accumulates errors for the row
  // below.
  int16_t *errCurr =
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = xStart; x != xEnd; x += dir) {
      // Add the accumulated diffusion error to the raw luminance and clamp to
      // [0, 255]
      ...
      if (mode == 0) {
        ...
        // Using integer rounding: (gray * 7 + 127) / 255 gives the nearest
        // level with no floating-point error and exact values at both
        // endpoints.
        quantLevel = (gray * 7 + 127) / 255;
        if (quantLevel > 7)
          quantLevel = 7;

        // Reconstruct the brightness that level actually represents on the
        // display. (quantLevel * 255 + 3) / 7 rounds to nearest, ensuring level
        // 7 maps exactly to 255 (no off-by-one that would leave a persistent
        // residual error).
      ...
      // Quantisation error: positive means the chosen level was darker than the
      // input
      int err = gray - quantGray;

      // Distribute the error to unprocessed neighbours using the
      // Floyd-Steinberg kernel. dir determines which neighbour is "forward" so
      // the kernel mirrors on odd rows.
      int xFwd = x + dir;
      int xBack = x - dir;
    ...
    // Swap the row buffers: the next row's accumulated errors become the
    // current ones
    int16_t *tmp = errCurr;
    errCurr = errNext;
    errNext = tmp;
