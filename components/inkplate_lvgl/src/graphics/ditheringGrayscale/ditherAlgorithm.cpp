 ...
 */
void DitherAlgorithm::begin(BoardBase *inkplatePtr)
{
...
void DitherAlgorithm::ditherFramebuffer(uint8_t *frameBuffer, int width, int height, uint8_t mode)
{
    ...
    int16_t *errCurr = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    int16_t *errNext = (int16_t *)heap_caps_malloc(width * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    if (!errCurr || !errNext)
    {
        free(errCurr);
        free(errNext);
